...
function divider (numbersArray){
  ...
  let fullArray = [evensArray, oddsArray]
  return fullArray
}
...
function removeItem (myGroceryList, remove) {
  let empty = []
  if (Boolean(myGroceryList)===false || Boolean(remove)===false) {
    return empty
  }
  else { for (i=0; i<myGroceryList.length;i++){
...
function addItem (myGroceryList, add)  {
  let empty = []
  if (Boolean(myGroceryList)===true && Boolean(add)===true) {
 ...
 console.log(addItem(myGroceryList, null))
...
function addTen (numbers){
  const tenArray = [] //numbers.slice(0,numbers.length)
  for (i=0;i<numbers.length;i++){
    tenArray.push(Number(numbers[i])+10)
  }
  return tenArray
}
...
function removeCahlan (devMountainEmployees){
  devMountainEmployees.filter(function(el){
    return el.name !== 'Cahlan'
  })
}
...
users.user2 = {
    name: 'Tyler McGinnis',
    email: 'tylermcginnis3s3@gmail.com',
    password: 'iLoveJavaScript',
    username: 'infiniteLoop'
}
users.user3 = {
    name: 'Tyler McGinnis',
    email: 'tylermcginnis3f3@gmail.com',
    password: 'iLoveJavaScript',
    username: 'infiniteLoop'
}
console.log(users)
